...
void parser_print_usage(void) {
    ...
    printf("  acp --tag \"NAME\"            Create a tag locally\n");
    printf("  acp --rm-tag \"NAME\"         Remove a tag locally\n");
    printf("  acp --tag-push \"NAME\"       Push a tag to the remote\n");
    printf("  acp --tag-rm-push \"NAME\"    Remove a tag from the remote\n");
    ...
    printf("  acp --tag \"v1.0.0\"\n");
    printf("  acp --tag-push \"v1.0.0\"\n");
}
...
int parser_parse(int argc, char **argv, AcpOptions *opts) {
    ...
    opts->tag_name = NULL;
    opts->tag_create_mode = 0;
    opts->tag_remove_mode = 0;
    opts->tag_push_mode = 0;
    opts->tag_remove_push_mode = 0;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--remote") == 0) {
            ...
            opts->tag_create_mode = 1;
            if (i + 1 < argc) {
                opts->tag_name = argv[i + 1];
                i++;
            } else {
                return -4;
            }
        } else if (strcmp(argv[i], "--rm-tag") == 0) {
            opts->tag_remove_mode = 1;
            if (i + 1 < argc) {
                opts->tag_name = argv[i + 1];
                i++;
            } else {
        ...
        } else if (strcmp(argv[i], "--tag-push") == 0) {
            opts->tag_push_mode = 1;
            if (i + 1 < argc) {
                opts->tag_name = argv[i + 1];
                i++;
            } else {
        ...
        } else if (strcmp(argv[i], "--tag-rm-push") == 0) {
            opts->tag_remove_push_mode = 1;
            if (i + 1 < argc) {
                opts->tag_name = argv[i + 1];
                i++;
            } else {
